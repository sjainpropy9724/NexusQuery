    ...
    try:
        for source in sources:
            try:
                ...
                # --- NEW BATCH GRAPH EXTRACTION STEP ---
                print(f"--- Celery: Starting BATCH graph extraction for {len(new_chunks)} chunks ---")
                graph_db.extract_and_store_graph_batch(new_chunks, kb_id)
                # --- END OF NEW STEP ---
